...
@time @sync @distributed for i=1:its
    ...
    #MEASURE SOMETHING
    mvj = dot(toothlength1[1,:],(toothdrop[:,2]/sum(toothdrop[:,2])))
    mva = dot(toothlength1[1,:],(toothdrop[:,1]/sum(toothdrop[:,1])));
...
filename = "figures/3oceans_means.pdf";
...
image.plot(x=$sigtauvec,y=$tauvec,z=$(meanjuv[1,1,:,:]),col=pal)
dev.off()
"""

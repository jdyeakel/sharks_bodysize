...
distvec = 700;
toothlossratepercentchangevec = collect(0.0:0.01:0.2);
...
# dist_posvec = repeat(collect(1:3),inner = length(sigtauvec)*length(tauvec));
...
ltl = length(toothlossratepercentchangevec);
paramvec_pre = repeat(paramposvec_pre,outer = ltl);
paramvec = [repeat(collect(1:ltl),inner=size(paramposvec_pre)[1]) paramvec_pre];
...
@time @sync @distributed for i=1:its
    ...
    # If not, continue
    if check == false
        println(string("running: ",[toothlossratepercentchangevec[toothlossratepercentchange_pos],sigtauvec[sigtau_pos],tauvec[tau_pos]]))
        ...
        popstate,
        toothdrop,
        state = popgen_migrate_g_toothseasonality(m0,M,tempvec1,tempvec2,n0,gen,distance,velocity,D,sigtau,tau,toothlossratepercentchange);
        ...
        println(string("saved: ",[toothlossratepercentchangevec[toothlossratepercentchange_pos],sigtauvec[sigtau_pos],tauvec[tau_pos]]))
    end

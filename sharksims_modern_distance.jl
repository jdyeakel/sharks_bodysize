...
ldist = length(distvec);
paramvec_pre = repeat(paramposvec_pre,outer = ldist);
paramvec = [repeat(collect(1:ldist),inner=size(paramposvec_pre)[1]) paramvec_pre];

its = size(paramvec)[1];

filename_settings = "data/sharks_modern2_dist/simsettings.jld";
namespace = smartpath(filename_settings);
@save namespace l0 L n0 gen mintemp_j maxtemp_j mintemp_a maxtemp_a distvec sigtauvec tauvec ldist paramvec its
